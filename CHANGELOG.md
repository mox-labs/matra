...
- Python: a caller's own object can be an embedder. `Matra.semantic_clusters(text, threshold, model)` takes a `Model2Vec` or any object with `embed` and `identity`, wrapped in the binding by an adapter over the `Embedder` port. The port was always the extension point; what was missing was the route to it from Python. The contract check stays in the library, so a Python embedder that returns the wrong number of vectors raises `ValueError` carrying the same message a Rust implementor would read, and an exception raised inside `embed` arrives as `ValueError` with its own text inside. The identity is read once, when the object is handed over, so scores cannot be reattributed mid-call. This arm needs no embedding adapter compiled in: a build without `model2vec` still clusters vectors a caller brings.
- Python: `Matra.analyze_path(path)` puts directory ingestion on the Python surface, `Ingest::path` into `Engine::analyze` collected in order. Each item is a `CorpusEntry` (`path`, `analysis`) or a `DocumentError` (`path`, `error`), so one unreadable file costs one item rather than the walk, and a failure listing the path itself raises instead. `DocumentError` wraps `std::io::Error` and has no serde form; the binding materializes its fields, with the error as a stable `kind` string plus the `Display` message.
- Python: `CorpusEntry`, `DocumentError`, `ErrorInfo` and the `CorpusItem` union join `matra.types`, along with the `Embedder` protocol; all five are exported from the package root and typed in `_core.pyi`.
