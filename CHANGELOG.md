...
- `NO_COLOR` (honored when present and not empty, per no-color.org) and `--color auto|always|never`, where an explicit choice outranks the variable in both directions. `--quiet` suppresses the human-readable output and leaves the exit code alone, with no effect on `--json`. `-` in place of a path reads stdin, with `--stdin-filename` supplying the label and the extension that selects the decomposer; the 8 MiB cap is enforced while reading rather than after it. `matra completions <bash|zsh|fish>` prints a completion script. `--version` names the version and then the features the build was compiled with.
- Python: `_core.cli_main(argv) -> int` runs the command line and returns its exit code, rendering into buffers that are then written to `sys.stdout` and `sys.stderr` so output interleaves correctly with anything Python already wrote.
