...
- `Model2Vec::potion_base_8m(dir)` and `Model2Vec::from_config(cfg)`: the reference embedding model without placing files by hand. The three artifacts are downloaded from URLs pinned in the source when they are absent, and nothing loads unless the SHA-256 over all three equals a constant pinned beside those URLs, so exactly one artifact set can arrive this way. Verification happens before parsing and the verified bytes are the parsed bytes, with no second read of the disk in between. A mismatch removes the files and downloads once more; a second mismatch removes them again and returns `Error::ModelInvalid`, so a partly-trusted model is never loaded and a failed attempt leaves nothing behind. Each artifact lands through a temporary file in the same directory and a rename, and a download past 64 MiB is `Error::InputTooLarge` with `what` set to `"embedding_download"`. `Model2Vec::from_dir` is unchanged and never downloads.
...
- `scripts/fetch-embedding-hash.sh`: fetches the three artifacts at a revision and prints the digest to paste into the adapter and into `spec/tests/semantic/reference-model.json`, the `scripts/fetch-model-hash.sh` counterpart for the embedding model.
