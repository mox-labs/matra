...
Future iterations are tracked in `.claude/implans/`. Past iterations are not rewritten; commitments only get superseded by new ADRs, never edited out.
...
2. Write a short proposal in `.claude/implans/` named `iN-<topic>.md`. Use the same structure as the existing implans.
3. If the change touches the public surface or a boundary rule, write an ADR under `docs/decisions/` and link it from the implan.
