...
- **You do not touch the real environment.** Sandbox the home directory, unset every `MATRA_*` variable, and confirm at the end that the real configuration and model directories are untouched. Say so in the report, with the warrant: a clean snapshot pair counts only when neither run printed a `WARNING`, because the snapshot does not see beneath a symlink, and a pass whose targets involve one runs in a container instead (see the sandbox contract in `.claude/skills/e2e-validation/SKILL.md`). Enter the sandbox with `eval "$(bash scripts/e2e-sandbox.sh new)"`; the script prints exports and moves nothing on its own, so running it without the `eval` runs your whole pass against your real home.
