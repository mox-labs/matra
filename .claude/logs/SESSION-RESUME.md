...
- **0.1.0 is published everywhere.** crates.io (maintainer-run `cargo publish`, 2026-08-23), PyPI (2026-09-04, all four artifacts), GitHub release v0.1.0. crates.io Trusted Publishing is configured (`publish.yml`, environment `crates-io`, required reviewers) and the maintainer was advised to enable require-trusted-publishing and revoke the bootstrap tokens. [Update 2026-09-20: `publish.yml` and `publish-pypi.yml` are now one `.github/workflows/release.yml`, dispatched from `main` rather than tag-triggered. The Trusted Publishing entry on crates.io and on PyPI names a workflow filename and must be repointed at `release.yml` before the next release, and the `pypi` environment needs required reviewers, which it has never had.]
