...
- **Scrub the home directory.** Point `HOME`, `XDG_CONFIG_HOME`, `XDG_DATA_HOME`, `XDG_CACHE_HOME` and `CARGO_HOME` inside a temporary tree and unset every `MATRA_*` variable. `eval "$(bash scripts/e2e-sandbox.sh new)"` does this, and the `eval` is the part that does it: the script only prints the exports and moves nothing itself, so running it without the `eval` leaves you in your real home and runs the whole pass against it. Take `bash scripts/e2e-sandbox.sh snapshot` before the pass and again after it, from outside the sandbox both times, and diff the pair. An identical pair **carrying no `UNREADABLE` line, from runs that printed no `WARNING`**, is evidence that the pass wrote nothing the snapshot can see. Both qualifiers are load-bearing. A target the command cannot read is recorded as unreadable in both halves, so the pair still diffs clean while that target went unexamined, and the report should say so. And the snapshot is a best-effort tripwire, not an isolation guarantee: it does not see beneath a symlink. A write through a target that is a symlink to a file, such as a `MATRA_CONFIG_FILE` pointing into a dotfiles repository, leaves the pair identical. So does any write inside a symlinked directory found within a target, other than adding, removing or renaming an entry directly in it, such as a model downloaded into a models directory linked onto another volume. The rule that follows: **if any target is or contains a symlink, a clean pair is not evidence of no leak, and the pass runs in the container layer instead.** `snapshot` prints a `WARNING` on stderr naming each such target, so read its stderr as well as the diff. The command refuses to run inside a sandbox, because in there it would fingerprint the sandbox and report a false all-clear. When it cannot read a target it writes `UNREADABLE` on that line and carries on to the rest, so a failure lands in the artifact being diffed rather than in an exit status the procedure never checks. `scripts/test-e2e-sandbox.sh` is the gate on all of that and runs from `just check`; `tests/sandbox_env_parity.rs` is the gate on the script naming every resolution tier `src/config.rs` reads.
- **Use a container for the install claim.** The maintainer's machine has a Rust toolchain, a C++ compiler, uv, certificates and cached models, so it structurally cannot answer whether a clean machine works. The pilot that found the glibc floor and the undocumented C++ requirement found both in containers, and the host could not have found either, because it has a C++ compiler and a glibc far newer than the floor. The missing ARM wheel is a weaker example and worth being honest about, because it was inferable from reading the publish workflow and the host pass read that same file and did not notice. The container is what made it impossible to overlook, not what made it findable. The container is also where the strong claim that nothing leaked belongs. A clean-room container never mounts the host's real configuration or data locations, so it cannot write to them, and that holds whatever is symlinked on the host, where the snapshot can only vouch for what it sees.
