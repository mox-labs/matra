...
- **A semgrep rule that silently matches nothing is the failure to fear.** Semgrep's Rust AST patterns miss brace groups and deep paths, its test discovery skips `.semgrep/`, and outside a git work tree its path-scoped rules match nothing. That is why every rule is a tested regex, why `scripts/check-boundaries.sh` pairs each rule file with its fixture itself, refuses to run outside a work tree, and fails unless the scan read every Rust file in `src/`. Run it through the script or `just boundary`, never as a bare `semgrep` call, and when adding a rule, plant a violation in the real file and watch it fail.
