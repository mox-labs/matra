...
**Code** honors the docs, and the enforcement is stated honestly rather than overclaimed. `scripts/check-boundaries.sh` checks the import forms of seven of the eight boundary rules with tested semgrep rules, and review reads their intent; the docsite floor checks link integrity, orphans, type-name parity, a clean site build, em dashes, `llms.txt` currency and the published URL manifest; both run from `just check` and in CI. Conventional commits map to CHANGELOG categories.
