...
#                            site/src/lib/figures/. Needs the UDPipe model,
...
# downloads the UDPipe model on first run, verified against the digest
# compiled into matra; with no model and no network it fails, never skips.
#
# Tunables:
...
if ! command -v cargo >/dev/null 2>&1; then
    ...
    fail=$((fail + 1))
elif ! cargo run --quiet --example docsite_figures -- --out "$fig_out" >"$fig_log" 2>&1; then
    echo "FAIL (gate 8): the figure generator did not run"
    echo "        (the UDPipe model is fetched on first use; without it this gate fails, it does not skip)"
    sed 's/^/  /' "$fig_log"
    fail=$((fail + 1))
    ...
    echo ""
    echo "        Run \`cargo run --example docsite_figures\` and review the change."
    fail=$((fail + 1))
fi
